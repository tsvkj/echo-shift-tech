...
export function ProjectCard({
  ...
  const main = project.images.find((i) => i.isMain) ?? project.images[0];
      ...
      <div className="p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {project.technologies.slice(0, 4).map((t) => (
            <span key={t} className="rounded-full bg-white/5 px-2 py-0.5 text-[11px] text-muted-foreground">
              {t}
            </span>
          ))}
        </div>
        ...
        <div className="mt-5 flex gap-2">
          ...
          >
            <Eye className="h-3.5 w-3.5" /> More details
          </button>
        </div>
      </div>
    </motion.article>
