...
export const Route = createFileRoute("/admin/login")({
  head: () => ({ meta: [{ title: "Admin · Nova.dev" }, { name: "robots", content: "noindex" }] }),
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-24">
      ...
      >
        <Link to="/" className="inline-flex items-center gap-2">
          ...
          <span className="font-display text-lg font-semibold">Nova.dev</span>
        </Link>

        <h1 className="mt-8 font-display text-2xl font-semibold">Admin access</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Frontend-only demo · replace with your JWT API.
        </p>

        <form onSubmit={onSubmit} className="mt-7 space-y-3">
          <label className="block rounded-2xl glass px-4 py-3">
            <span className="block text-[10px] uppercase tracking-widest text-muted-foreground">Email</span>
            <input
              type="email"
              value={email}
          ...
          <label className="block rounded-2xl glass px-4 py-3">
            <span className="block text-[10px] uppercase tracking-widest text-muted-foreground">Password</span>
            <input
              type="password"
              value={password}
        ...
        <div className="mt-5 rounded-xl border border-white/5 bg-white/5 p-3 text-xs text-muted-foreground">
          <span className="text-primary">Demo:</span> {authService.demoCredentials.email} /{" "}
