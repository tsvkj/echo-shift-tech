...
export const Route = createFileRoute("/admin/")({
  head: () => ({ meta: [{ title: "Dashboard · Nova.dev" }, { name: "robots", content: "noindex" }] }),
...
function AdminDashboard() {
  ...
  const refresh = () => projectsService.list().then(setItems);
  ...
  const onSave = async (draft: Omit<Project, "id" | "createdAt"> & { id?: string }) => {
    if (editing) {
      await projectsService.update(editing.id, draft);
    } else {
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        ...
        <header className="flex items-center justify-between rounded-2xl glass-strong p-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={async () => { await projectsService.reset(); refresh(); }}
        ...
        <div className="mt-6 overflow-hidden rounded-2xl glass">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                >
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      {p.images[0]?.url ? (
                        <img src={p.images[0].url} alt="" className="h-10 w-14 rounded-md object-cover" />
                      ) : (
                  ...
                  <td className="p-4">
                    {p.featured ? (
                      <span className="rounded-full bg-primary/15 px-2.5 py-0.5 text-xs text-primary">Yes</span>
                    ) : (
                  ...
                  <td className="p-4 text-right">
                    <div className="inline-flex gap-1">
                      <button onClick={() => setEditing(p)} className="flex h-8 w-8 items-center justify-center rounded-md glass hover:bg-primary/10">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      <button onClick={() => onDelete(p.id)} className="flex h-8 w-8 items-center justify-center rounded-md glass text-destructive hover:bg-destructive/20">
              ...
              {items.length === 0 && (
                <tr><td colSpan={4} className="p-10 text-center text-sm text-muted-foreground">No projects yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
